import asyncio

from sqlalchemy import desc, select

from app.db.engine import engine
from app.db.models import journal_entries, mood_logs


async def fetch_sample_records(limit: int = 5) -> dict:
    async with engine.begin() as conn:
        journal_result = await conn.execute(
            select(journal_entries)
            .order_by(desc(journal_entries.c.created_at))
            .limit(limit)
        )
        journals = [dict(row._mapping) for row in journal_result.fetchall()]

        mood_result = await conn.execute(
            select(mood_logs)
            .order_by(desc(mood_logs.c.timestamp))
            .limit(limit)
        )
        moods = [dict(row._mapping) for row in mood_result.fetchall()]

        return {"journals": journals, "mood_logs": moods}


if __name__ == "__main__":

    async def main():
        records = await fetch_sample_records()
        print("Latest Journal Entries:")
        for entry in records["journals"]:
            print(entry)
        print("Latest Mood Logs:")
        for mood in records["mood_logs"]:
            print(mood)

    asyncio.run(main())
